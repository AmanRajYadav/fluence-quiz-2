...
const DailyQuiz = () => {
  ...
  // Robust question loading with multiple fallback methods
  useEffect(() => {
    ...
    // TRY MULTIPLE APPROACHES TO FETCH
    const tryFetch = async () => {
      // Method 1: Direct public folder access
      try {
      ...
      // Method 2: Try with process.env.PUBLIC_URL
      try {
        const response = await fetch(`${process.env.PUBLIC_URL}/${questionFile}`);
        if (response.ok && response.headers.get('content-type')?.includes('application/json')) {
          const lastModified = response.headers.get('last-modified');
          if (lastModified) {
            setQuizUpdateDate(new Date(lastModified));
      ...
      // If all methods fail
      if (!isMounted) return;
      setQuestionLoadError('Could not load quiz questions. Please check file locations.');
      setQuestions([]);
    };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-600 via-blue-600 to-indigo-800 flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 max-w-md w-full text-center shadow-2xl border border-white/20">
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-white mb-2 animate-pulse flex items-center justify-center gap-2">
              <Target className="w-8 h-8 text-pink-400 inline-block" /> Daily Quiz
            </h1>
